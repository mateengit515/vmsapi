...
public interface VoterRepository extends JpaRepository<Voter, Integer> {
    ...
    @Query(value = "SELECT door_no as doorNo, house_total as houseTotal, incharge as incharge FROM ( SELECT door_no, house_total, incharge, ROW_NUMBER() OVER (PARTITION BY door_no ORDER BY house_total DESC) AS rn FROM voters ) t WHERE rn = 1 ORDER BY house_total DESC, door_no ASC", nativeQuery = true)
